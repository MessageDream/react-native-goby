...
@implementation GobyPackage

#pragma mark - Private constants

static NSString *const DiffManifestFileName = @"hotgoby.json";
static NSString *const DownloadFileName = @"download.zip";
static NSString *const RelativeBundlePathKey = @"bundlePath";
static NSString *const StatusFile = @"goby.json";
...
+ (void)clearUpdates
{
    [[NSFileManager defaultManager] removeItemAtPath:[self getGobyPath] error:nil];
}
           ...
           failCallback:(void (^)(NSError *err))failCallback
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:newUpdateFolderPath]) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:newUpdateFolderPath
                                                   error:&error];
    } else if (![[NSFileManager defaultManager] fileExistsAtPath:[self getGobyPath]]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:[self getGobyPath]
                                  withIntermediateDirectories:YES
                                                   attributes:nil
                                                        error:&error];
        
        // Ensure that none of the Goby updates we store on disk are
        // ever included in the end users iTunes and/or iCloud backups
        NSURL *gobyURL = [NSURL fileURLWithPath:[self getGobyPath]];
        [gobyURL setResourceValue:@YES forKey:NSURLIsExcludedFromBackupKey error:nil];
    }
    ...
    GobyDownloadHandler *downloadHandler = [[GobyDownloadHandler alloc]
                                                init:downloadFilePath
                                                operationQueue:operationQueue
                                                progressCallback:progressCallback
                                                doneCallback:^(BOOL isZip) {
                                                    NSError *error = nil;
                                                    NSString * unzippedFolderPath = [GobyPackage getUnzippedFolderPath];
                                                    ...
                                                    if (isZip) {
                                                        ...
                                                        if (isDiffUpdate) {
                                                            ...
                                                            if (currentPackageFolderPath == nil) {
                                                                ...
                                                                NSString *newUpdateGobyPath = [newUpdateFolderPath stringByAppendingPathComponent:[GobyUpdateUtils manifestFolderPrefix]];
                                                                //NSString *newUpdateGobyPath = newUpdateFolderPath;
                                                                [[NSFileManager defaultManager] createDirectoryAtPath:newUpdateGobyPath
                                                                                          withIntermediateDirectories:YES
                                                                                                           attributes:nil
                                                                                                                error:&error];
                                                                if (error) {
                                                                ...
                                                                [[NSFileManager defaultManager] copyItemAtPath:[Goby bundleAssetsPath]
                                                                                                        toPath:[newUpdateGobyPath stringByAppendingPathComponent:[GobyUpdateUtils assetsFolderName]]
                                                                                                         error:&error];
                                                                if (error) {
                                                                    failCallback(error);
                                                                    return;
                                                                ...
                                                                [[NSFileManager defaultManager] copyItemAtPath:[[Goby binaryBundleURL] path]
                                                                                                        toPath:[newUpdateGobyPath stringByAppendingPathComponent:[[Goby binaryBundleURL] lastPathComponent]]
                                                                                                         error:&error];
                                                                if (error) {
                                                                    failCallback(error);
                                                                    return;
                                                        ...
                                                        [GobyUpdateUtils copyEntriesInFolder:unzippedFolderPath
                                                        ...
                                                        NSString *relativeBundlePath = [GobyUpdateUtils findMainBundleInFolder:newUpdateFolderPath
                                                        ...
                                                        if (relativeBundlePath) {
                                                            ...
                                                            NSString *errorMessage = [NSString stringWithFormat:@"Update is invalid - A JS bundle file named \"%@\" could not be found within the downloaded contents. Please ensure that your app is syncing with the correct deployment and that you are releasing your Goby updates using the exact same JS bundle file name that was shipped with your app's binary.", expectedBundleFileName];
                                                            
                                                            error = [GobyErrorUtils errorWithMessage:errorMessage];
                                                        ...
                                                        if (isDiffUpdate && ![GobyUpdateUtils verifyHashForDiffUpdate:newUpdateFolderPath
                                                            ...
                                                            error = [GobyErrorUtils errorWithMessage:@"The update contents failed the data integrity check."];
...
+ (NSString *)getGobyPath
{
    NSString* gobyPath = [[Goby getApplicationSupportDirectory] stringByAppendingPathComponent:@"Goby"];
    if ([Goby isUsingTestConfiguration]) {
        gobyPath = [gobyPath stringByAppendingPathComponent:@"TestPackages"];
    }
...
{
    NSString *packageHash = [GobyPackage getCurrentPackageHash:error];
    if (!packageHash) {
        return nil;
    }
    
    return [GobyPackage getPackage:packageHash error:error];
}
...
{
    return [[self getGobyPath] stringByAppendingPathComponent:DownloadFileName];
}
...
{
    return [[self getGobyPath] stringByAppendingPathComponent:packageHash];
}
...
{
    return [[self getGobyPath] stringByAppendingPathComponent:StatusFile];
}

+ (NSString *)getUnzippedFolderPath
{
    return [[self getGobyPath] stringByAppendingPathComponent:UnzippedFolderName];
}
