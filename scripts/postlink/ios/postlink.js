...
https://github.com/buxuxiao/react-native-goby#plugin-configuration-ios`);
    return;
}
...
var gobyHeaderImportStatement = `#import <Goby/Goby.h>`;
if (~appDelegateContents.indexOf(gobyHeaderImportStatement)) {
    console.log(`"Goby.h" header already imported.`);
} else {
...
var newJsCodeLocationAssignmentStatement = "jsCodeLocation = [Goby bundleURL];";
if (~appDelegateContents.indexOf(newJsCodeLocationAssignmentStatement)) {
    console.log(`"jsCodeLocation" already pointing to "[Goby bundleURL]".`);
} else {
...
// 3. Add GobyDeploymentKey to plist file
var parsedInfoPlist = plist.parse(plistContents);
if (parsedInfoPlist.GobyDeploymentKey) {
    console.log(`"GobyDeploymentKey" already specified in the plist file.`);
    writePatches();
} else {
    inquirer.prompt({
        "type": "input",
        "name": "iosDeploymentKey",
        "message": "What is your Goby deployment key for iOS (hit <ENTER> to ignore)"
    }).then(function(answer) {
        parsedInfoPlist.GobyDeploymentKey = answer.iosDeploymentKey || "deployment-key-here";
