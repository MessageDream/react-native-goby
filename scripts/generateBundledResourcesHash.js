...
var GOBY_FOLDER_PREFIX = "Goby";
var GOBY_HASH_FILE_NAME = "GobyHash";
var GOBY_HASH_OLD_FILE_NAME = "GobyHash.json";
var HASH_ALGORITHM = "sha256";
var TEMP_FILE_PATH = path.join(require("os").tmpdir(), "GobyResourcesMap.json");
...
function addJsBundleAndMetaToManifest() {
    addFileToManifest(path.dirname(jsBundleFilePath), path.basename(jsBundleFilePath), manifest, function() {
        ...
        addFileToManifest(path.dirname(jsBundleMetaFilePath), path.basename(jsBundleMetaFilePath), manifest, function() {
            ...
            var savedResourcesManifestPath = assetsDir + "/" + GOBY_HASH_FILE_NAME;
            fs.writeFileSync(savedResourcesManifestPath, finalHash);

            var oldSavedResourcesManifestPath = assetsDir + "/" + GOBY_HASH_OLD_FILE_NAME;
...
function addFileToManifest(folder, assetFile, manifest, done) {
    ...
    readStream.pipe(hashStream)
        ...
        .on("finish", function() {
            ...
            manifest.push(path.join(GOBY_FOLDER_PREFIX, assetFile).replace(/\\/g, "/") + ":" + fileHash);
            done();
        });
}
