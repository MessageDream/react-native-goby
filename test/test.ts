...
/// <reference path="../node_modules/goby-plugin-testing-framework/typings/goby-plugin-testing-framework.d.ts" />
...
import { Platform, PluginTestingFramework, ProjectManager, setupTestRunScenario, setupUpdateScenario, ServerUtil, TestBuilder, TestConfig, TestUtil } from "goby-plugin-testing-framework";
...
class RNAndroid extends Platform.Android implements RNPlatform {
    ...
    installPlatform(projectDirectory: string): Q.Promise<void> {
        ...
        //// Set up gradle to build Goby with the app
        // Add Goby to android/app/build.gradle
        var buildGradle = path.join(innerprojectDirectory, "android", "app", "build.gradle");
        TestUtil.replaceString(buildGradle,
            "apply from: \"../../node_modules/react-native/react.gradle\"",
            "apply from: \"../../node_modules/react-native/react.gradle\"\napply from: \"" + path.join(innerprojectDirectory, "node_modules", "react-native-goby", "android", "goby.gradle") + "\"");
        TestUtil.replaceString(buildGradle,
            "compile \"com.facebook.react:react-native:+\"",
            "compile \"com.facebook.react:react-native:0.25.+\"");
        TestUtil.replaceString(buildGradle,
            "// From node_modules",
            "\n    compile project(':react-native-goby') // From node_modules");
        // Add Goby to android/settings.gradle
        ...
        var mainActivity = path.join(innerprojectDirectory, "android", "app", "src", "main", "java", "com", "buxuxiao", "goby", "test", "MainActivity.java");
        TestUtil.replaceString(mainActivity, TestUtil.GOBY_TEST_APP_NAME_PLACEHOLDER, TestConfig.TestAppName);
...
class RNIOS extends Platform.IOS implements RNPlatform {
    ...
    installPlatform(projectDirectory: string): Q.Promise<void> {
        ...
        return TestUtil.getProcessOutput("pod init", { cwd: iOSProject })
            ...
            .then<void>(TestUtil.replaceString.bind(undefined, appDelegatePath, TestUtil.GOBY_TEST_APP_NAME_PLACEHOLDER, TestConfig.TestAppName));
    }
...
class RNProjectManager extends ProjectManager {
    ...
    public setupScenario(projectDirectory: string, appId: string, templatePath: string, jsPath: string, targetPlatform: Platform.IPlatform, version?: string): Q.Promise<void> {
        ...
        return TestUtil.copyFile(templateIndexPath, destinationIndexPath, true)
            .then<void>(TestUtil.replaceString.bind(undefined, destinationIndexPath, TestUtil.GOBY_TEST_APP_NAME_PLACEHOLDER, TestConfig.TestAppName))
            ...
            .then<void>(TestUtil.replaceString.bind(undefined, destinationIndexPath, TestUtil.GOBY_APP_VERSION_PLACEHOLDER, version));
    }

    /**
     * Creates a Goby update package zip for a project.
     */
    public createUpdateArchive(projectDirectory: string, targetPlatform: Platform.IPlatform, isDiff?: boolean): Q.Promise<string> {
        var bundleFolder: string = path.join(projectDirectory, TestConfig.TestAppName, "Goby/");
...
PluginTestingFramework.initializeTests(new RNProjectManager(), supportedTargetPlatforms,
    (projectManager: ProjectManager, targetPlatform: Platform.IPlatform) => {
        TestBuilder.describe("#window.goby.checkForUpdate",
            () => {
                TestBuilder.it("window.goby.checkForUpdate.noUpdate", false,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.goby.checkForUpdate.sendsBinaryHash", false,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.goby.checkForUpdate.noUpdate.updateAppVersion", false,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.goby.checkForUpdate.update", true,
                    (done: MochaDone) => {
                ...
                TestBuilder.it("window.goby.checkForUpdate.error", false,
                    (done: MochaDone) => {
        ...
        TestBuilder.describe("#window.goby.checkForUpdate.customKey",
            () => {
                TestBuilder.it("window.goby.checkForUpdate.customKey.update", false,
                (done: MochaDone) => {
                    var updateResponse = ServerUtil.createUpdateResponse();
                    ServerUtil.updateResponse = { updateInfo: updateResponse };
        ...
        TestBuilder.describe("#goby.restartApplication",
            () => {
                TestBuilder.it("goby.restartApplication.checkPackages", true,
                    (done: MochaDone) => {
                        ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
        ...
        TestBuilder.describe("#goby.restartApplication.2x",
            () => {
        ...
        TestBuilder.describe("#window.goby.sync",
            () => {
                ...
                TestBuilder.describe("#window.goby.sync 1x",
                    () => {
                        // Tests where sync is called just once
                        TestBuilder.it("window.goby.sync.noupdate", false,
                            (done: MochaDone) => {
                                var noUpdateResponse = ServerUtil.createDefaultResponse();
                                noUpdateResponse.isAvailable = false;
                                noUpdateResponse.appVersion = "0.0.1";
                        ...
                        TestBuilder.it("window.goby.sync.checkerror", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = "invalid {{ json";
                        ...
                        TestBuilder.it("window.goby.sync.downloaderror", false,
                            (done: MochaDone) => {
                        ...
                        TestBuilder.it("window.goby.sync.dorevert", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
                        ...
                        TestBuilder.it("window.goby.sync.update", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
                ...
                TestBuilder.describe("#window.goby.sync 2x",
                    () => {
                        // Tests where sync is called again before the first sync finishes
                        TestBuilder.it("window.goby.sync.2x.noupdate", false,
                            (done: MochaDone) => {
                                var noUpdateResponse = ServerUtil.createDefaultResponse();
                                noUpdateResponse.isAvailable = false;
                                noUpdateResponse.appVersion = "0.0.1";
                        ...
                        TestBuilder.it("window.goby.sync.2x.checkerror", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = "invalid {{ json";
                        ...
                        TestBuilder.it("window.goby.sync.2x.downloaderror", false,
                            (done: MochaDone) => {
                                var invalidUrlResponse = ServerUtil.createUpdateResponse();
                                invalidUrlResponse.downloadURL = path.join(TestConfig.templatePath, "invalid_path.zip");
                                ServerUtil.updateResponse = { updateInfo: invalidUrlResponse };
                        ...
                        TestBuilder.it("window.goby.sync.2x.dorevert", false,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
                        ...
                        TestBuilder.it("window.goby.sync.2x.update", true,
                            (done: MochaDone) => {
                                ServerUtil.updateResponse = { updateInfo: ServerUtil.createUpdateResponse(false, targetPlatform) };
        ...
        TestBuilder.describe("#window.goby.sync minimum background duration tests",
            () => {
        ...
        TestBuilder.describe("#window.goby.sync mandatory install mode tests",
            () => {
