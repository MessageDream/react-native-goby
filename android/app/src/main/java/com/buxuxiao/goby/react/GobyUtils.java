...
public class GobyUtils {
    ...
    public static WritableArray convertJsonArrayToWritable(JSONArray jsonArr) {
        ...
        for (int i=0; i<jsonArr.length(); i++) {
            ...
            try {
                ...
                // Should not happen.
                throw new GobyUnknownException(i + " should be within bounds of array " + jsonArr.toString(), jsonException);
            }
            ...
            if (obj instanceof JSONObject)
                ...
                throw new GobyUnknownException("Unrecognized object: " + obj);
        }
    ...
    public static WritableMap convertJsonObjectToWritable(JSONObject jsonObj) {
        ...
        while(it.hasNext()){
            ...
            try {
                ...
                // Should not happen.
                throw new GobyUnknownException("Key " + key + " should exist in " + jsonObj.toString() + ".", jsonException);
            }
                ...
                case Number:
                    ...
                    if ((number == Math.floor(number)) && !Double.isInfinite(number)) {
                        ...
                        try {
                            ...
                            throw new GobyUnknownException("Unable to put value " + arr.getDouble(i) + " in JSONArray");
                        }
                    }
                    break;
                case Boolean:
            ...
            try {
                switch (type) {
                    ...
                    case Null:
                        ...
                        break;
                    default:
                        throw new GobyUnknownException("Unrecognized type: " + type + " of key: " + key);
                }
            } catch (JSONException jsonException) {
                throw new GobyUnknownException("Error setting key: " + key + " in JSONObject", jsonException);
            }
        }
    ...
    public static JSONObject getJsonObjectFromFile(String filePath) throws IOException {
        ...
        try {
            ...
            // Should not happen
            throw new GobyMalformedDataException(filePath, jsonException);
        }
    }

    public static void log(String message) {
        Log.d(GobyConstants.REACT_NATIVE_LOG_TAG, "[Goby] " + message);
    }
    ...
    public static void setJSONValueForKey(JSONObject json, String key, Object value) {
        try {
            ...
            throw new GobyUnknownException("Unable to set value " + value + " for key " + key + " to JSONObject");
        }
    }
